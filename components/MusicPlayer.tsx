...
export function MusicPlayer({ playlist, onSongChange }: MusicPlayerProps) {
  ...
  const handleDoubleClick = () => {
    if (!isDragging) { // Prevent double click while dragging
      setIsMini(!isMini)
  ...
  const handleDragStop = () => {
    // Add small delay to prevent double click trigger after drag
    setTimeout(() => setIsDragging(false), 100)
  }

  const playerContent = (
    <div 
      className={`w-full max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-4 sm:p-6 music-player-transition
    ...
    >
      <div className={`flex ${isMini ? 'flex-col items-center' : 'flex-col sm:flex-row sm:items-center sm:space-x-4'}`}>
        ...
        <div className={`flex-grow text-center ${isMini ? 'title-container' : 'sm:text-left'}`}>
          <h2 className={`font-bold truncate ${isMini ? 'title-text' : 'text-xl'}`}>
            {currentSong?.title}
          </h2>
          <p className={`text-gray-600 truncate ${isMini ? 'hidden' : ''}`}>
            {currentSong?.artist}
          </p>
          <p className={`text-gray-500 truncate ${isMini ? 'hidden' : ''}`}>
            {currentSong?.album}
          </p>
        </div>
      </div>

      {/* Hide these sections in mini mode */}
